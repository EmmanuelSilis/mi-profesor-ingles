...
export default function GuidedWriting({course}:{course:Course}) {
  ...
  return <div className="space-y-4"><div className="flex gap-3"><button className="action" aria-pressed={!exact} onClick={()=>setExact(false)}>Respuestas con estructura</button><button className="rounded-lg border p-3 bg-white" aria-pressed={exact} onClick={()=>setExact(true)}>Preguntas de repaso</button></div>{exact ? <CoursePractice course={course} mode="escribir"/> : <section className="panel space-y-5">
    <h1 className="text-2xl font-bold">Escribir con pistas</h1><p>Practica los temas de tu curso con pistas y explicaciones. La corrección usa patrones guiados; no analiza cualquier frase libre.</p>
    <label className="block">Tema para repasar<select className="block border rounded-lg p-2 w-full" value={topic} onChange={e=>{setTopic(e.target.value);setIndex(0);setAnswer('');setGrade(null);}}>{topics.map(t=><option key={t}>{t}</option>)}</select></label>{all.some(e=>e.kind==='grammar') && <button className="underline text-blue-700" onClick={()=>{setVariant(v=>v+1);setIndex(0);setAnswer('');setGrade(null);}}>Cambiar nombres de los ejemplos de nacimiento y origen</button>}
    <label className="block">Elegir ejercicio<select className="block border rounded-lg p-2 w-full" value={index} onChange={e=>{setIndex(Number(e.target.value));setAnswer('');setGrade(null);}}>{exercises.map((x,i)=><option key={x.id} value={i}>{i+1}. {x.prompt}</option>)}</select></label>
    <p className="text-sm text-slate-600">{exercise.context}</p><h2 className="text-xl font-semibold">{exercise.prompt}</h2>
    <ProgressiveHints key={`${variant}-${exercise.id}`} exercise={exercise} />

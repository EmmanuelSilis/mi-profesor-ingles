...
interface Draft { id:string; name:string; preview:string; pages:SourcePage[]; error?:string; saved?:boolean }
export default function PdfImport(){
 ...
 onDrop:async files=>{if(!files.length||lock.current)return;lock.current=true;setBusy(true);setError('');
  try{for(let i=0;i<files.length;i++){const file=files[i],id=crypto.randomUUID(),preview=URL.createObjectURL(file);urls.current.push(preview);setProgress(`Archivo ${i+1}/${files.length}: ${file.name}`);
   try{const result=file.name.toLowerCase().endsWith('.pdf')?await extractPdfContent(file,p=>setProgress(`${file.name}: ${p.stage} · página ${p.page}/${p.total}`)):await extractImageContent(file);setDrafts(s=>[...s,{id,name:file.name,preview,pages:result.sourcePages}]);}catch(e){setDrafts(s=>[...s,{id,name:file.name,preview,pages:[],error:e instanceof Error?e.message:'No se pudo leer.'}]);}
  }}finally{lock.current=false;setBusy(false);}},
 });
 const saveDraft=(draft:Draft)=>{try{if(!draft.pages.some(p=>p.text.trim()))throw new Error('No hay texto legible.');save(analyzeCourse(draft.pages,draft.name));setDrafts(ds=>ds.map(d=>d.id===draft.id?{...d,saved:true,error:undefined}:d));}catch(e){setError(e instanceof Error?e.message:'No se pudo guardar.');}};

...
export function combineCourses(courses: Course[], selections: Selection[], title: string): Course {
 const id=crypto.randomUUID(); const pages:SourcePage[]=[], cards:Course['cards']=[], units:Course['units']=[], basicQuestions:NonNullable<Course['basicQuestions']>=[];
 for(const choice of selections){
  ...
  cards.push(...course.cards.filter(c=>selected.has(c.page)).map(c=>({...c,id:`${id}-${c.id}`})));
  basicQuestions.push(...(course.basicQuestions || []).filter(q=>selected.has(q.page)).map(q=>({...q,id:`${id}-${q.id}`})));
 ...
 return {id,fileName:title.trim()||'Repaso combinado',pages,cards,units,basicQuestions,selections,createdAt:new Date().toISOString()};
}
